import { IconGithub, IconLinkedin } from '../assets/icons';

interface SocialLink {
  name: string;
  url: string;
  icon: React.ReactNode;
}

interface SocialLinksProps {
  className?: string;
}

export const SocialLinks = ({ className = '' }: SocialLinksProps) => {
  const socials: SocialLink[] = [
    {
      name: 'GitHub',
      url: 'https://github.com/denzellgh',
      icon: <IconGithub className="h-5 w-5" />,
    },
    {
      name: 'LinkedIn',
      url: 'https://www.linkedin.com/in/denzell-griffith',
      icon: <IconLinkedin className="h-5 w-5" />,
    },
  ];

  return (
    <div className={`flex gap-4 ${className}`}>
      {socials.map(social => (
        <a
          key={social.name}
          href={social.url}
          target="_blank"
          rel="noopener noreferrer"
          className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-accent-blue transition-all duration-300 hover:scale-110 hover:bg-accent-blue/10 hover:shadow-[0_0_20px_rgba(14,165,233,0.4)]"
          aria-label={social.name}
        >
          {social.icon}
        </a>
      ))}
    </div>
  );
};
